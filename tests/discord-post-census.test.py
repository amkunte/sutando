...
# FORK DEBT — senders that predate upstream's chokepoint and still call the API
# directly. Listed explicitly so the census keeps enforcing the invariant for
# every OTHER file instead of being switched off wholesale. These are NOT
# readers: they are not held to the read-only check below, because they do post.
# Route each through src/channels/discord/client.py, then delete it from here.
FORK_SENDERS = {
    "src/discord_post.py",
    "skills/charlie-node/scripts/charlie_relay.py",
    "skills/charlie-node/scripts/charlie_thread_read.py",
    "skills/exec-approval/scripts/common.py",
    "skills/finance-radar/scripts/common.py",
}
...
for rel, f in production_files():
    ...
    try:
        ...
        continue
    if "discord.com/api" in text and rel not in ALLOWED and rel not in FORK_SENDERS:
