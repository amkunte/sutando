...
1. **Check for tasks.** Look in `tasks/` for voice / Discord / Telegram / phone tasks. Look at `context-drop.txt` for context drops. Process anything found — execute the task, write results to `results/`.
   - **Fidelity gate:** before writing the result, run step 6.6 against it — tag claims MEASURED/INFERRED, do not assert a cause formed this pass without falsifying it, and red-team anything that recommends an action or asserts a root cause.
...
2.6. **Skill-scan catch-up (self-healing).** Run `python3 src/scan-catchup.py`. The skill scans — amazon-orders (#orders), parcel-radar (#parcels), trip-radar (#travel), karts-air (#deals) — are session-only CronCreate jobs that die on session end/compaction and auto-expire after 7 days; during a long unattended window they silently stop and the channels go quiet (`last_scan` stops advancing). This script reads each scan's `last_scan` on disk and prints `SCANDUE <name> :: <hint>` for any scan overdue past its cadence (amazon/parcel 6h, trip/karts 24h, ×1.5 grace). For each `SCANDUE` line, **run that scan now** per the hint's scan-prompt (which updates `last_scan`, making this idempotent — a scan that ran on time won't re-fire, a missed one recovers within one pass). This is the durable backstop that makes silent-stop impossible: the trigger is `last_scan` on disk, not a cron, so cron expiry / session restart can't stop it. Silent output = nothing overdue. Roaming nodes (Maverick, traveling with the owner) set `SKIP_SKILL_SCANS=1` so only the home-base node (Goose) runs scans — no double-posting.

   **The script prints three markers, and each needs a different response. Two of them had no rule here at all until 2026-08-21, which is a silent-failure shape: an LLM consumer given an unrecognized line either ignores it (the signal is lost) or pattern-matches it onto the only actionable rule above and runs the scan — the exact double-post the roaming gate exists to prevent.**

   - `SCANDUE <name> :: <hint>` — **run that scan now** (as above). Self-clearing: the scan advances `last_scan`.
   - `SCANBLOCKED <name> :: …` — the scan RAN and every source was blocked, so `last_scan` is fresh and the age check cannot see it. **Do NOT re-run it** — re-running reproduces the nothing. Surface it to the owner.
   - `SCANSTALE <name> :: …` — emitted only on a roaming node (`SKIP_SKILL_SCANS=1`). State pulled via fleet-sync has not advanced. **Do NOT run the scan here** — this node is gated out precisely so exactly one node scans, and a second runner double-posts to the channel. It is an observation with two candidate causes (owner node stopped / fleet-sync stopped); surface both, do not assert one.

   **`SCANBLOCKED` and `SCANSTALE` repeat on every pass and cannot self-clear from this node** — the roaming node can never advance `last_scan` (fleet-sync only pushes items it owns), so an untreated stall re-prints ~150×/day at a 10-minute cadence. Do NOT surface them per-pass. Route them through the **step 6.5 surface-once-per-changed-set** path: fold the marker lines into the held-list, hash it, and post only when the hash changes. Non-silent output therefore no longer means "something is overdue for this node" — it means "read the marker".
 (Added 2026-06-20 after #orders went silent ~2 days mid-trip; see build_log.)
...
6.6. **FIDELITY GATE — run before anything reaches the owner (task reply, DM, pending-question, recommendation).** Added 2026-08-13 after the owner said he was losing trust in the fidelity of my recommendations. That day: 49 build-log entries, 64 lines of correction language, 5 catches by a peer rather than by me — while the `red-team` skill was invoked **0 times** and sandboxed second-opinion **0 times**, despite a standing memory saying to always run both. **Seven memories already existed about this exact failure class and did not prevent it.** Memories are passive (read at session start, maybe); this gate is in-context at act-time, every pass. That difference is the whole point — do not "fix" a recurrence by writing an eighth memory.

   **(a) Tag every claim by evidence class.** MEASURED = I can paste the command and its output. INFERRED = reasoning from a measurement. ASSUMED = neither. The day's dominant failure was a correct measurement welded to an invented cause — *"the index was replaced"* (measured) *"by the peer's sync"* (invented, refuted). Fluent prose makes the two halves indistinguishable to the reader **and to the writer**. If a claim is INFERRED and would change what the owner does, either verify it first or ship it wearing the label.

   **(b) Never report a causal claim in the same pass it was formed.** Form it → try to falsify it → then report. In-session proof: the PR #2757 "the branch is stale" diagnosis explained every symptom, was strongly evidenced, and was **half wrong** — and it is the ONLY major claim that day which never reached the owner wrong, precisely because the rebase was run and re-measured before speaking. Every other error was sent immediately and corrected minutes later **by my own next command** — the tell that it was reported early, not thought about too little.

   **(c) Adversarial pass is mandatory, not discretionary, for decision-changing artifacts.** Trigger — not a judgment call, since that judgment is what failed: any artifact that (i) recommends an action, (ii) asserts a root cause, or (iii) precedes an irreversible operation → invoke the `red-team` skill (or delegate a second opinion via `claude-codex`) BEFORE sending. Budget note: this costs quota and lowers output volume. That is the intended trade — fewer, right.

   **Two mechanical habits that catch the recurring shapes cheaply:** print the denominator beside every difference count (a zero from a command that never ran looks identical to a real zero), and state the search scope out loud before reporting an absence (a too-narrow search once caused a true finding to be *retracted*).
