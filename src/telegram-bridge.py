    ...
    while True:
        ...
        if result.get("ok"):
            ...
            for update in result.get("result", []):
                ...
                # Local (#4): match on numeric id OR @username — allowFrom entries
                # were historically written as either form, and an id-only check
                # silently dropped owners enrolled under their handle.
            ...
            if not presenter_mode_active(REPO):
                ...
                for f in RESULTS_DIR.iterdir():
                    if any(f.name.startswith(p) for p in PROACTIVE_PREFIXES) \
                        ...
                        if owner_id is None:
                            # Preserve, never drop: health-check's proactive-quarantine
                            # probe reads results/undelivered/, and leaving the body in
                            # the poll glob re-reads it every cycle instead.
