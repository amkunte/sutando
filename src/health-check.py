...
def _file_unchanged_since(src_file: Path, proc_start: float) -> bool:
    ...
    try:
        ...
        show = subprocess.run(
            git_argv("show", f"{old_head}:{rel_path}"),
...
def check_core_proactive_loop(threshold_sec: int = 600, idle_threshold_sec: int = 1800) -> dict:
    ...
    ts = data.get("ts")
    if state == "idle":
        # "idle" is written at the END of every pass, so a loop that never woke again
        # leaves a permanently-valid-looking "idle". Only a fresh one is healthy.
...
def check_core_supervisor(supervisor_stale_sec: int = 600) -> dict:
    ...
    # a monitor that died months ago still reports its last state as current.
    try:
...
def run_all_checks() -> list[dict]:
    ...
    # warn for a proxy it is not meant to run. Resolve via claude_home_path(),
    # not ~/.claude: post-#1454 the skills dir lives under the workspace.
    ...
    # NOTE: local `skip_telegram` was dropped in the 2026-07-23 upstream merge —
    # `_should_skip_bridge(channel_name, env_path)` is called unconditionally at
    ...
    for name, proc_name in [("telegram-bridge", "telegram-bridge"), ("discord-bridge", "discord-bridge"),
                            ("sms-bridge", "sms-bridge"), ("slack-bridge", "slack-bridge")]:
        ...
        # A co-hosted Charlie discord-bridge legitimately runs a 2nd
        # discord-bridge.py against a non-default channels dir. Drop those PIDs
        # UP FRONT so a co-host can neither mask the PRIMARY being down nor
        # inflate the duplicate count (masked a 19-min outage, 2026-06-12).
