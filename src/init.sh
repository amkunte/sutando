...
preflight() {
  ...
  # macOS permission probes — non-fatal hints. Both invoke a GUI subsystem:
  # screencapture talks to the window server, osascript talks to System Events.
  # In a headless context (launchd background job, SSH without an Aqua login, or
  # a Mac with no attached display / no console user) there is nothing to answer
  # them and they BLOCK — hanging the whole startup. Gate on an Aqua (GUI)
  # session: Aqua present ⇒ window server present ⇒ probes are safe; Aqua absent
  # ⇒ headless ⇒ skip them so startup never stalls. (Hung the Goose Mac-mini
  # setup 2026-06 — see build_log.)
  #
  # macOS 15+ silently writes a tiny PNG when screen-recording perm is denied
  # (exit 0). Denied artifacts are <2KB; real captures are hundreds-of-KB to MB.
  # Black 5120x2880 PNG compresses to ~43KB, so 5KB is the safe floor.
  local perms_warn=0
  local perms_skipped=0
  if [ "$(launchctl managername 2>/dev/null)" = "Aqua" ]; then
    ...
    # 125 = could not check. acc_rc=0 here would BE the granted verdict.
    local acc_rc=125
    if [ -f "$__probe" ]; then
    ...
    perms_skipped=1
    log "  ⓘ GUI permission checks skipped (headless — no Aqua session)"
  fi
  ...
  if [ "$perms_skipped" -eq 1 ]; then perms_str="skipped-headless"; fi
