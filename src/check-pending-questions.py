...
def get_waiting_questions():
    ...
    for sec in sections[1:]:  # skip pre-header
        ...
        # scaffolding prose lives below the rule, not in any question's body. (ours)
        body = re.split(r'^---\s*$', body, maxsplit=1, flags=re.MULTILINE)[0]
        # Title filter is UPSTREAM's, deliberately replacing our
        # `re.search(r'\b(resolved|done|answered)\b', title)`: that crude keyword
        # search deleted LIVE questions (a title like "render a [DONE] badge" is a
        # real ask). _ORG_HEADING/_INLINE_RESOLVED are anchored + shape-based —
        # see their definitions above for the incidents that motivated them.
        if _ORG_HEADING.match(title) or _INLINE_RESOLVED.match(title):
            continue
        # ALSO keep our keyword filter: this fork's file uses a TRAILING marker
        # (`## Foo — RESOLVED 2026-08-13`), which _INLINE_RESOLVED does not match
        # (it requires a BRACKETED marker leading the title). Dropping it took the
        # live count 6 -> 10 by un-hiding already-resolved entries. Upstream's
        # matchers are kept for the bracketed/organizer forms they were written for.
        if re.search(r'\b(resolved|done|answered)\b', title, re.IGNORECASE):
            continue
        # Body handling is OURS (owner decision 2026-08-13): a section with a
        # TITLE but no body IS a pending question — the title carries the ask.
        # Only an explicit "(none)" placeholder is skipped. The placeholder regex
        # is all-optional so it also matches ""; the `body_text and` guard is what
        # stops an empty body being swallowed by it.
