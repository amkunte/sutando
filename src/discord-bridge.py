...
CHANNELS_DIR = Path(
    os.environ.get(
        ...
        # Default honors the workspace-scoped CLAUDE_CONFIG_DIR resolution
        # (claude_home_path, post-#1454) instead of a hardcoded ~/.claude path;
        # the SUTANDO_DISCORD_CHANNELS_DIR override still lets an isolated
        # co-host (Charlie) point elsewhere.
        str(claude_home_path("channels", "discord")),
    )
).expanduser()
...
# Sourced from CHANNELS_DIR (local #104), NOT a hardcoded
# claude_home_path("channels","discord"): a co-hosted bridge (Charlie) runs with
# SUTANDO_DISCORD_CHANNELS_DIR pointing elsewhere, and hardcoding the default
# would chmod/read the personal bot's .env instead of its own.
...
async def _handle_discord_message(message, force=False):
    ...
    if sender_id in allowed:
        ...
        try:
            ...
            # Deliberately AFTER the team branch so peer wins for this channel.
    ...
    # headers written by the bridge BEFORE the user-supplied `task:` body (which
    # stays last per the field-order invariant), so confine_user_content defangs
    # any forged copy a user smuggles in the body but leaves these intact.
    ...
    while True:
        ...
        for task_id in list(pending_replies.keys()):
            ...
            if result_file.exists():
                ...
                # Upstream still uses a bare pop here — do NOT take theirs.
            ...
            for f in RESULTS_DIR.iterdir():
                ...
                if f.name.startswith("proactive-") and f.suffix == ".txt" \
                    ...
                    if owner_id is None:
                        ...
                        # Body already preserved above, so the fence call only
                        # completes the delivery record — its unlink is a no-op.
