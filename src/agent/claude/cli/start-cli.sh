...
# when it's up, so quota tracking (state/quota-state.json) stays fresh (#123).
...
# left the 5h window stuck at a past reset time for hours). Passed to tmux via
...
if [ -x "$REPO/scripts/sutando-config.sh" ]; then
  ...
  if _ccd="$(bash "$REPO/scripts/sutando-config.sh" claude-sutando-config-dir 2>"$_ccd_err")"; then
    ...
    if [ -n "$PY" ] && "$PY" -c 'import sys' > /dev/null 2>&1; then
      _ccd="$_ccd" _cwd="${SUTANDO_CLAUDE_WORKING_DIR:-}" _apikey="${ANTHROPIC_API_KEY:-}" _accept_bypass="${SUTANDO_ACCEPT_BYPASS_PERMISSIONS:-}" "$PY" - <<'PY' || echo "  ⚠ onboarding-seed skipped (non-fatal)"
import json, os
ccd = os.environ["_ccd"]
...
if [ -n "$RESTART_REQUESTED" ]; then
  ...
  if tmux_session_exists || core_claude_running; then
    ...
    # still-dying core slip into the "orphan reuse → exit 0" guard below. Poll ~3s.
        # Window widened 3s -> 10s from our 4184b19b: `claude` reliably takes
        # longer than 3s to exit, and a still-dying core makes --restart tear the
        # core down then decline to start a new one, leaving NO core (hit twice
        # 2026-08-13). Upstream's FORCE_RESTART escalation below is kept.
